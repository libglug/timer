...
void continuous_timer_alloc(const struct glug_allocator *alloc, struct glug_continuous_timer **timer)
{
...
void continuous_timer_free(struct glug_continuous_timer **timer)
{
    (*timer)->base.free(*timer);
    *timer = NULL;
}

void continuous_timer_start(struct glug_continuous_timer *timer)
{
    timer_start(&timer->base, read_continuous_ticks);
}

void continuous_timer_pause(struct glug_continuous_timer *timer)
{
    timer_pause(&timer->base, read_continuous_ticks);
}

void continuous_timer_reset(struct glug_continuous_timer *timer)
{
    timer_reset(&timer->base, read_continuous_ticks);
}

void continuous_timer_delta(struct glug_continuous_timer *timer, struct glug_time *res)
{
    timer_delta(&timer->base, read_continuous_ticks, res);
}

void continuous_timer_run_time(struct glug_continuous_timer *timer, struct glug_time *res)
{
    timer_run_time(&timer->base, read_continuous_ticks, res);
}

void continuous_timer_resolution(struct glug_time *res)
{
    continuous_clock_res(res);
}

enum glug_timer_state continuous_timer_state(const struct glug_continuous_timer *timer)
{
